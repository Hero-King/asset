...
# 安装nodejs
node_url="https://mirrors.huaweicloud.com/nodejs/latest-v12.x/node-v12.19.1-linux-x64.tar.xz"
cd /usr/local/ && wget ${node_url}  && xz -d node-v12.19.1-linux-x64.tar.xz && \
tar xvf node-v12.19.1-linux-x64.tar &&  mv node-v12.19.1-linux-x64 node-v12.19.1 && \
echo "export NODE_HOME=/usr/local/node-v12.19.1" >> /etc/profile && source /etc/profile && echo "export PATH=$PATH:$NODE_HOME/bin " >> /etc/profile && source /etc/profile
