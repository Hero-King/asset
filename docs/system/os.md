...
启动： CPU / 内存 / IO 通过总线连接
接电之后 执行内存中特定地址的 BIOS 程序（自检显卡，外设正常和执行 BIOS（加载硬盘中 bootloader（第一个启动扇区））） 加载内核
...
系统调用：
应用程序发出调用；操作系统去处理
API ： win32 API 用于 windows POSIX API 用于 linux unix macos , javaapi 用于 jvm (jvm 在调用 win32 或者 posix)
...
三种方式: socket 远程过程调用(rpc) java 的远程方法调用(RMI)
